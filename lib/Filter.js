...
function Filter (config, httpFetcher) {
...
Filter.prototype.setProperties = function (query, cb) {
    var self = this;
    this._wheelchair_accessible = query.wheelchair_accessible;

    //check if needed to prefetch data
    if (this._wheelchair_accessible && !this._data_prefetched ) {

        //prefetch trips data
        console.log('Getting page: ' + this._config.wheelchair_trips);
        this._http.get(this._config.wheelchair_trips).then(function(result) {
            ...
            for (var i = 0, len = trips_no_wha.length; i < len; i++) {
                self._no_wha_trip_ids.push(trips_no_wha[i].trip_id);
            }
