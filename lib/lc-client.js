...
var Client = function (config) {
  ...
  // Create http fetcher (also needed for getTransferTime)
  this._http = new HttpFetcher(20); // 20 concurrent requests max.

  // Create fetcher
  this._fetcher = new Fetcher(this._config, this._http);
 ...
 */
Client.prototype.getTransferTime = function (perviousConnection, connection, minTransferTime, wheelchairAccessible, cb){
  if(this._transfer_times_prefetched){
    this._getTransferTimeAndTransferDataIsPrefetched(perviousConnection, connection, minTransferTime, wheelchairAccessible, cb);
    ...
    this._http.get(this._config.transfer_times).then( function(result) {
      ...
      self._getTransferTimeAndTransferDataIsPrefetched(perviousConnection, connection, minTransferTime, wheelchairAccessible, cb);
    });
  }
}
 ...
 */
Client.prototype._getTransferTimeAndTransferDataIsPrefetched = function (perviousConnection, connection, minTransferTime, wheelchairAccessible, cb){
  var self= this;
  var transfertime = 0;
