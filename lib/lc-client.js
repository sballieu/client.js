 ...
 */
Client.prototype.getTransferTime = function (perviousConnection, connection, minTransferTime, wheelchairAccessible, done){
  var self= this;
  var transfertime = 0;
  ...
  if(perviousConnection && perviousConnection["trip"]!=null && perviousConnection["trip"] != connection["gtfs:trip"]["@id"]) {
    ...
    else
    {
      ...
      var _checkWheelchairAccessible = function (actualTransferTime, departureStop, done) {
      ...
      if(this._wha_data_prefetched) {
        ...
        _checkWheelchairAccessible(transfertime,connection["departureStop"],done);
