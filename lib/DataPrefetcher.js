...
var DataPrefetcher = function(config) {
    ...
    this._no_wha_trip_ids = undefined; //list of trip ids that are not wheelchair accessible
}
 ...
 */
DataPrefetcher.prototype.isTripWheelchairAccessible = function (tripid){
    var self = this;

    if(!this._no_wha_trip_ids) {
        this._no_wha_trip_ids = new Promise(function (fulfill, reject){
            //prefetch trips data

            console.log('Getting page: ' + self._config.wheelchair_trips);
            self._http.get(self._config.wheelchair_trips).then(function(result) {

                var trips = JSON.parse(result.body)["@graph"];

                // filter the non wheelchair accessible trips
                var trips_no_wha = trips.filter(function (trip) {
                    return (trip.wheelchair_accessible == "gtfs:NotWheelchairAccessible");
                });
                ...
                for (var i = 0, len = trips_no_wha.length; i < len; i++) {
                    no_wha_trip_ids.push(trips_no_wha[i].trip_id);
                }

                fulfill(no_wha_trip_ids);
            });
        });
    }

    return new Promise(function (fulfill, reject) {
        self._no_wha_trip_ids.then(function (no_wha_trip_ids) {
            fulfill(!(no_wha_trip_ids.indexOf(tripid) > - 1));
        });
    });
}
