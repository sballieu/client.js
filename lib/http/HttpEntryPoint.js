...
var HttpEntryPoint = function (entryPoint, http, onlyWheelchairAccessibleTrips) {
  EventEmitter.call(this);
  this._http = http;
  this._entryPoint = entryPoint;
  this._onlyWheelchairAccessibleTrips = onlyWheelchairAccessibleTrips;
}
  ...
  return this._http.get(this._entryPoint).then(function (result) {
    ...
    return jsonld.promises.frame(document, {
      ...
      if (data["@graph"][0] && data["@graph"][0].search && data["@graph"][0].search.template) {
        ...
        var params = {};
        params[data["@graph"][0].search.mapping[0].variable] = date.toISOString();
        if(self._onlyWheelchairAccessibleTrips) {
          params[data["@graph"][0].search.mapping[1].variable] = true;
        }
