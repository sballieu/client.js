...
var Fetcher = function (config, http, dataPrefetcher) {
...
Fetcher.prototype.buildConnectionsStream = function (query, cb) {
  ...
  async.forEachOf(this._entrypoints, function (entryUrl, i, done) {
    new HttpEntryPoint(entryUrl, self._http)
      ...
      .then(function (url) {
        ...
        if (i === 0) {
          connectionsStream = newConnectionsStream.pipe(filter);
          self._connectionsStreams = [newConnectionsStream];
        } else {
          self._connectionsStreams.push(newConnectionsStream.pipe(filter));
