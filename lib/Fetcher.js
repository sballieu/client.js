...
var Fetcher = function (config) {
  ...
  this.filter = new Filter(this._config, this.http);
}
...
Fetcher.prototype.buildConnectionsStream = function (query, cb) {
  ...
  this.filter.setProperties(query, function() {
    ...
    for (var k in self._entrypoints) {
      var entry = new HttpEntryPoint(self._entrypoints[k], self.http);
      entry.fetchFirstUrl(query.departureTime).then(function (url) {
        ...
        if (self._connectionsStreams.length === 1) {
          cb(self._connectionsStreams[0][1].pipe(self.filter)); // Only one stream
          ...
          cb(self._mergeStream.pipe(self.filter));
        }
      }, function (error) {
        console.error(error);
      });
